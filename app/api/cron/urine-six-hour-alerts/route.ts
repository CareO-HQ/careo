...
export async function GET(request: NextRequest) {
  try {
    ...
    if (residentsError) {
      console.error("Cron urine reminder resident query failed:", residentsError);
    ...
    const { data: existingAlertRows, error: existingAlertsError } = await supabase
      ...
      .eq("type", URINE_NOT_RECORDED_6H_ALERT_TYPE)
      .eq("is_resolved", false)
      .in("resident_id", residentIds);

    if (existingAlertsError) {
      console.error("Cron urine reminder existing alert query failed:", existingAlertsError);
      return NextResponse.json({ error: "Failed to query existing alerts" }, { status: 500 });
    }

    const { data: urineRows, error: urineRowsError } = await supabase
    ...
    if (urineRowsError) {
      console.error("Cron urine reminder continence query failed:", urineRowsError);
      return NextResponse.json({ error: "Failed to query urine entries" }, { status: 500 });
    }

    const latestUrineByResident = new Map<string, string>();
    for (const urineRow of (urineRows as UrineEntryRow[] | null) ?? []) {
      if (!latestUrineByResident.has(urineRow.resident_id)) {
        latestUrineByResident.set(urineRow.resident_id, urineRow.created_at);
      }
    }
    ...
    const alertsToResolve = existingAlerts
      .filter((alert) => {
        ...
        const lastUrineCreatedAt = latestUrineByResident.get(alert.resident_id);
        if (!lastUrineCreatedAt) {
          return false;
        }
        return new Date(lastUrineCreatedAt).getTime() > new Date(alert.created_at).getTime();
      })
      .map((alert) => alert.id);
    ...
    if (alertsToResolve.length > 0) {
      ...
      if (resolveError) {
        console.error("Cron urine reminder alert resolve failed:", resolveError);
    ...
    const alertsToInsert = residents
      .filter((resident) => {
        ...
        const lastUrineCreatedAt = latestUrineByResident.get(resident.id);
        if (!lastUrineCreatedAt) {
          return true;
        }
        const inactivityDurationMs = nowMs - new Date(lastUrineCreatedAt).getTime();
        return inactivityDurationMs >= URINE_ALERT_WINDOW_MS;
      })
      .map((resident) => {
        ...
        const lastUrineAt = latestUrineByResident.get(resident.id) ?? null;

        return {
          resident_id: resident.id,
          organization_id: resident.organization_id,
          care_home_id: resident.care_home_id,
          type: URINE_NOT_RECORDED_6H_ALERT_TYPE,
          severity: "warning",
          title: "No urine recorded in 6 hours",
          message: `No urine entry has been recorded for ${residentName} in the last 6 hours.`,
          metadata: {
            generated_by: "urine-six-hour-alert-cron",
            threshold_hours: 6,
            last_urine_recorded_at: lastUrineAt,
    ...
    if (alertsToInsert.length > 0) {
      ...
      if (insertError) {
        console.error("Cron urine reminder alert insert failed:", insertError);
    ...
    console.error("Cron urine reminder execution failed:", error);
