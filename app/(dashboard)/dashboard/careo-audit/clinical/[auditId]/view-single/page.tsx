...
function ClinicalAuditViewSinglePageContent() {
  const params = useParams();
  const router = useRouter();
  // auditId parameter actually contains the responseId for this page
  const responseId = params.auditId as Id<"clinicalAuditCompletions">;

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [actionPlanToDelete, setActionPlanToDelete] = useState<Id<"clinicalAuditActionPlans"> | null>(null);

  // Load the completed response from database
  const dbResponse = useQuery(
    api.clinicalAuditResponses.getResponseById,
    { responseId }
  );
  ...
  const deleteActionPlan = useMutation(api.clinicalAuditActionPlans.deleteActionPlan);
  ...
  const openDeleteDialog = (planId: string) => {
    setActionPlanToDelete(planId as Id<"clinicalAuditActionPlans">);
