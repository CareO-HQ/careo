...
export default function ClinicalAuditPage() {
  ...
  const updateTemplate = useMutation(api.clinicalAuditTemplates.updateTemplate);
  ...
  // Clinical template IDs start with letters like 'v', 'k', 'j', etc.
  ...
  const existingDrafts = useQuery(
    api.clinicalAuditResponses.getDraftResponsesByTemplate,
  ...
  const getOrCreateDraft = useMutation(api.clinicalAuditResponses.getOrCreateDraft);
  const updateResponse = useMutation(api.clinicalAuditResponses.updateResponse);
  const completeAudit = useMutation(api.clinicalAuditResponses.completeAudit);

  // State for tracking current audit response ID
  const [responseId, setResponseId] = useState<Id<"clinicalAuditCompletions"> | null>(null);
  ...
  useEffect(() => {
    console.log("🔍 Checking for existing clinical drafts:", {
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        console.log("💾 Auto-saving clinical audit data...");
      ...
      <div className="flex items-center justify-between border-b px-6 py-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Clinical Care & Medicines</p>
          </div>
        </div>
      </div>
