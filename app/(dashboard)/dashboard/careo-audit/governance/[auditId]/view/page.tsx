...
function GovernanceAuditViewPageContent() {
    ...
    const checkAuditId = async () => {
      try {
        ...
        if (completionData) {
          ...
          router.replace(`/dashboard/careo-audit/governance/${completionData.template_id}/view`);
          return;
        }
  ...
  useEffect(() => {
    ...
    const fetchTemplate = async () => {
      try {
        const templateData = await auditService.getGovernanceTemplateById(templateId);
        setTemplate(templateData);
      } catch (error) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {template?.name || "Governance Audit"}
            </p>
          </div>
        </div>
