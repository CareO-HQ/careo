...
function EnvironmentAuditViewPageContent() {
    ...
    const checkAuditId = async () => {
      try {
        ...
        if (completionData) {
          ...
          router.replace(`/dashboard/careo-audit/environment/${completionData.template_id}/view`);
          return;
        }
  ...
  useEffect(() => {
    ...
    const fetchTemplate = async () => {
      try {
        const templateData = await auditService.getEnvironmentTemplateById(templateId);
        setTemplate(templateData);
      } catch (error) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {template?.name || "Environment Audit"}
            </p>
          </div>
        </div>
