  ...
  const handleSubmit = async (data: SocialActivityFormData) => {
    ...
    try {
      ...
      form.reset({
        ...
        socialInteraction: "active",
        enjoyment: "enjoyed",
        recordedBy: profile.name || profile.email?.split('@')[0] || "",
      });
                      ...
                      <div className="flex items-start space-x-3 flex-1">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-1">
                            ...
                            <Badge
                              className={`text-xs border-0 ${activity.activityType === "group_activity" ? "bg-purple-100 text-purple-800" :
                ...
                {socialConnections.map((connection) => (
                  <div key={connection._id} className="p-4 border rounded-lg">
                    <div className="flex items-center space-x-3 mb-2">
                      <div className={`p-2 rounded-lg ${connection.type === 'family' ? 'bg-blue-100' :
                        connection.type === 'friend' ? 'bg-green-100' :
                          connection.type === 'staff' ? 'bg-purple-100' :
                            'bg-gray-100'
                        }`}>
                        <User className={`w-4 h-4 ${connection.type === 'family' ? 'text-blue-600' :
