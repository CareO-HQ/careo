...
import { ArrowLeft, Phone, Calendar, MapPin, AlertTriangle, Heart, Activity } from "lucide-react";
...
export default function ResidentPage({ params }: ResidentPageProps) {
  ...
  const resident = useQuery(api.residents.getById, {
    residentId: params.id as Id<"residents">
  });
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">The resident youre looking for doesnt exist.</p>
          <Button 
            variant="outline" 
            className="mt-4"
            onClick={() => router.back()}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Detailed information about {fullName}</p>
          </div>
        </div>
      </div>
              ...
              <div className="space-y-3">
                ...
                {resident.nhsHealthNumber && (
                  <div className="flex items-center space-x-2 text-sm">
                    ...
                    <span className="font-mono">{resident.nhsHealthNumber}</span>
                  </div>
                )}
              </div>
            </CardContent>
                ...
                <div className="space-y-3">
                  {resident.emergencyContacts.map((contact, index) => (
                    <div key={index} className="space-y-1">
                      ...
                      {index < resident.emergencyContacts.length - 1 && <Separator className="mt-3" />}
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(resident.healthConditions) && typeof resident.healthConditions[0] === 'string' ? (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {Array.isArray(resident.risks) && typeof resident.risks[0] === 'string' ? (
              ...
              <CardContent>
                {Array.isArray(resident.dependencies) ? (
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {Object.entries(resident.dependencies as { mobility: string; eating: string; dressing: string; toileting: string }).map(([key, value]) => (
                      <div key={key} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                        <span className="font-medium capitalize">{key}</span>
                        <Badge 
                          variant={value === 'Independent' ? 'secondary' : 'outline'}
                          className="text-xs"
                        >
                          {value}
                        </Badge>
