-- Force restructure of capacity_consents to ensure assessment_data exists
-- This is a safety migration because 20260319144400 seems to not have applied the JSONB change correctly on some environments

DO $$ 
BEGIN
    -- If assessment_data doesn't exist, we likely have the old flattened version
    IF NOT EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_schema = 'public' 
        AND table_name = 'capacity_consents' 
        AND column_name = 'assessment_data'
    ) THEN
        -- Drop and recreate to match the standard JSONB pattern
        DROP TABLE IF EXISTS public.capacity_consents CASCADE;
