...
export const getActionPlansByAudit = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!role) {
      console.warn(`[RBAC] Access denied: User attempted to view care file action plans but user role not found`);
      return [];
    }

    if (!canViewAudit(role)) {
      console.warn(`[RBAC] Access denied: User attempted to view care file action plans for audit ${args.auditResponseId} but role ${role} cannot view audits`);
      return [];
    }
...
export const getActionPlansByTemplate = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!canViewAudit(role)) {
      console.warn(`[RBAC] Access denied: User attempted to view care file action plans for template ${args.templateId} but role ${role} cannot view audits`);
      return [];
    }
...
export const getCreatedActionPlans = query({
  ...
  handler: async (ctx, args) => {
    ...
    const { role } = await resolveUser(ctx);
    if (!role) {
      console.warn(`[RBAC] Access denied: User attempted to view created care file action plans but user role not found`);
      return [];
    }

    if (!canViewAudit(role)) {
      console.warn(`[RBAC] Access denied: User attempted to view created care file action plans but role ${role} cannot view audits`);
      return [];
    }
...
export const getActionPlanById = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (actionPlan.assignedTo === user.email) {
      ...
      console.warn(`[RBAC] Access denied: User attempted to view care file action plan ${args.actionPlanId} but user role not found`);
      return null;
    } else if (!canViewAudit(role)) {
      console.warn(`[RBAC] Access denied: User attempted to view care file action plan ${args.actionPlanId} but role ${role} cannot view audits`);
      return null;
    }
...
export const getActionPlansByTeam = query({
  ...
  handler: async (ctx, args) => {
    ...
    const { role } = await resolveUser(ctx);
    if (!role) {
      console.warn(`[RBAC] Access denied: User attempted to view care file action plans for team ${args.teamId} but user role not found`);
      return [];
    }

    if (!canViewAudit(role)) {
      console.warn(`[RBAC] Access denied: User attempted to view care file action plans for team ${args.teamId} but role ${role} cannot view audits`);
      return [];
    }
...
export const getOverdueActionPlans = query({
  ...
  handler: async (ctx, args) => {
    ...
    const { role } = await resolveUser(ctx);
    if (!role) {
      console.warn(`[RBAC] Access denied: User attempted to view overdue care file action plans for team ${args.teamId} but user role not found`);
      return [];
    }

    if (!canViewAudit(role)) {
      console.warn(`[RBAC] Access denied: User attempted to view overdue care file action plans for team ${args.teamId} but role ${role} cannot view audits`);
      return [];
    }
...
export const getActionPlanStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const { role } = await resolveUser(ctx);
    if (!role) {
      console.warn(`[RBAC] Access denied: User attempted to view care file action plan stats for team ${args.teamId} but user role not found`);
      return {
        total: 0,
        pending: 0,
        inProgress: 0,
    ...
    if (!canViewAudit(role)) {
      console.warn(`[RBAC] Access denied: User attempted to view care file action plan stats for team ${args.teamId} but role ${role} cannot view audits`);
      return {
        total: 0,
        pending: 0,
        inProgress: 0,
