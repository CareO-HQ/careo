  ...
  handler: async (ctx, args) => {
    ...
    const enrichedPlans = await Promise.all(
      actionPlans.map(async (plan) => {
        ...
        return {
          ...plan,
          templateName: template?.name || "Unknown Clinical Audit",
  ...
  handler: async (ctx, args) => {
    ...
    if (actionPlan.createdBy !== updatedBy) {
      ...
      await ctx.db.insert("notifications", {
        ...
        message: `${updatedByName || "An assignee"} updated the action plan status to "${status}" for clinical audit "${template?.name || "audit"}": "${actionPlan.description}"${comment ? `\n\nComment: ${comment}` : ""}`,
        link: `/dashboard/careo-audit/clinical/${actionPlan.auditResponseId}/view`,
        metadata: {
